...
- **Whole-document reading** — `R` turns a Notes page or a Terminal window into an audiobook, starting where the mouse points. PDFs in Preview read page-aware: `Ctrl+F`/`Ctrl+B` turn pages, `12G` jumps to page twelve. Web pages read in Safari and Firefox — open the Reader view first and it's just the title and article, no site clutter. And the view follows the voice: Preview turns to the page you jump to, Reader articles scroll along as the read moves (`:config follow off` to keep the view still).
...
- **Dark-mode survival for bright apps (opt-in)** — list an app (Packet Tracer, Pages) and Marduk flips the system's Invert Colors when it comes front, restoring on the way out; `:config pdfdark on` switches every Preview PDF to its dark view automatically; `:config autoinvert on` goes further and *measures* the front window's brightness with a tiny screenshot (Screen Recording permission), inverting only when the content is actually bright — your black-styled slide deck stays exactly as you made it.
