...
- **Image descriptions** — point at a picture (a photo in Messages, an image on a web page, a file open in Preview) and press `D` to hear what it shows and any text in it; `rr` repeats it. Apple's own on-device model describes it on macOS 27 with Apple Intelligence; on today's macOS a local [Ollama](https://ollama.com) vision model (`gemma3:4b`, the one Marduk's news triage already uses) writes the description, and without either, Apple's Vision framework names what it sees, counts faces, and reads the text. `:config imagemodel` picks the engine. Images never leave your Mac — the capture is of the window under the pointer only, and only a local Ollama is ever used.
