...
- **Automation** — the first time speech ducks your media, allow the prompts to control System Events / Music / Spotify. The news reader asks for Terminal the first time you press `n`, and the daily brief asks for Notes the first time it reads your note.
- **Screen Recording** (opt-in) — asked for the first time you press `D` on a picture that has to be captured from the screen, or turn on `:config smartinvert`. Marduk opens the pane and talks you through it.
...
- **Image descriptions, and questions about them** — point at a picture (a photo in Messages, an image on a web page, a file open in Preview) and press `D` to hear what it shows, in your reading voice: what kind of image, the subject, the setting, any text exactly as written, and if it is a meme or a joke, what the joke is. The description ends with "Do you have any questions?" — press `y`, type a question, press Return, and the answer comes back about that same picture, with the earlier questions remembered ("and the one on the left?"); `:ask` works any time the picture is still remembered. `DD` describes your **whole screen**: every window front to back, any dialog or alert you may not know is there, the menu bar and the Dock — assembled window by window, so it is the whole display even when you are zoomed in. `:config detail brief|normal|full` sets how much a description says, and `v`, `vv`, `vvv` re-describe the current picture at that level on the fly; `:config prompt` replaces Marduk's prompt with your own words. Apple's on-device model does the describing on macOS 27 with Apple Intelligence; on today's macOS a local [Ollama](https://ollama.com) vision model (`gemma3:4b`, the one Marduk's news triage already uses) writes it, and without either, Apple's Vision framework names what it sees, counts faces, and reads the text. `:config imagemodel` picks the engine. Images never leave your Mac — the capture is of the window under the pointer only, a Marduk-started Ollama is shut down after two idle minutes, and only a local Ollama is ever used.
- **News, read to you** — `n` opens your [newsboat](https://newsboat.org) feeds in a Terminal window with a spoken mirror over it: `j`/`k` walk the feeds and articles by title, `Enter` opens one, `r` reads the whole article through the long-form reader (so every reading motion works on it), `o` opens it in the browser, `d` deletes, `/` searches, `y` copies its link, `Escape` leaves. Feeds refresh on the way in, and a `git`-managed feed list pulls before every open. `t` sends the newest unread headlines to a local Ollama model, which picks the three that matter and groups the duplicates — speak it, press 1, 2, or 3, and Marduk takes you to the story. Needs newsboat (`brew install newsboat`); Ollama is optional. A one-line key bar shows Marduk's keys for the sighted half of a low-vision user.
- **A spoken stock watchlist** — `S` opens your watchlist: `j`/`k` speak each ticker with its price and the day's change in words ("309 dollars 86 cents, up 1.2 percent"), `r` the full quote, `a` adds a ticker, `dd` removes one, `b` and `s` set buy and sell alerts that Marduk announces the moment the price crosses them — it never trades. Quotes come from a keyless public endpoint; the list lives in `~/.config/marduk/stocks.json`. A terminal-style panel mirrors it on screen while it is open.
- **A daily brief** — `d` reads your morning in one go: the date and time, the weather for your city (`:config place`, no account and no location permission), a note of your own from Notes (`:config note` names it), your watchlist with any alert levels already crossed, and the newest unread headlines; the moon phase and a horoscope from any feed you subscribe to are there if you turn them on. It is a read, so Space pauses it, `}` skips to the next part, and `rr` says it again. `:segments` chooses what goes in.
...
| `marduk ax-probe` | Dump the accessibility element under the pointer (image-description diagnostics) |
...
**COMMAND mode (`:`):** type `:` in NORMAL for a vim-style command line with a floating palette showing your options. `:help` speaks the basics, `:commands` the full reference, `:tutorial` starts the guided tour, `:tip` speaks a random feature tip, `:quit` / `:restart` control the daemon, `:update` installs updates, `:uninstall` removes the launch agent, `:log` opens the log, `:feedback` / `:bug` open GitHub issues, `:voices` opens the voice picker (each candidate previews in its own voice), `:invertapps` chooses which apps invert the display — the app you were just in comes first, so Return alone adds it; typing filters, and Return on an app already on the list removes it — `:pronunciation` opens the system pronunciation editor, `:describe` describes the picture under the pointer and `:ask <question>` asks about the last one, `:news`, `:brief`, and `:stock add|remove|buy|sell` reach the news reader, the daily brief, and the watchlist from anywhere, `:segments` chooses the brief's parts, `:karabiner` applies your own Karabiner rules from `~/.config/marduk/ke/rules.json`, and `:config <setting> <value>` changes settings live — `rate` (50–360 wpm), `pitch` (50–200%), `level` (`none`/`some`/`most`/`all`), `hashes`, `identifiers` (speak `parseHTMLBody` as separate words), `rescue`, `burst`, `escapehold`, `echo` (speak keys as you type, off by default), `commandecho`, `palette`, `position` (`pointer`/`center`), `autoupdate`, `checkhours`, `border`, `pointer`, `thickness`, `speedkeys` (Option+arrows nudge the rate), `togglesound` (`speech`/`earcon`), `readmotions`, `follow` (scroll the view to track the read), `dialogs` (`all`/`system`/`off`), `dialogfocus` (`ask`/`always`/`off` — whether Marduk may jump focus to an announced dialog), `hints` (occasional feature tips), `invert`, `pdfdark` (`auto`/`on`/`off`), `autoinvert`, `dock`, for image descriptions `describe` (on/off), `imagemodel` (`auto`/`apple`/`ollama`/`labels`), `detail` (`brief`/`normal`/`full`), and `prompt <your own words>`; the extension switches `news`, `stocks`, and `brief` (off returns each key to its old meaning); and the brief's setup — `place <city>`, `units` (`imperial`/`metric`), `note <title>`, `headlines` (0–20), and `horoscope <part of a feed name>`. Everything else takes `on`/`off` or a number. Unique prefixes work everywhere (`:conf ra 230`); Tab completes; `?` or a moment's pause speaks what you can type next; Escape cancels.
...
- `describe` — image descriptions: engine (`imageModel`), detail level, your own prompt, and an Ollama vision model / URL override (defaults to the news block's)
- `news` — the newsboat handoff: the command to run, and optional urls/cache/config file overrides for a private newsboat instance (`~/.config/marduk/news/` is used automatically when it holds a `urls` file), plus the Ollama model and URL for `t` triage
- `brief` — the daily brief: which segments and in what order, the place and coordinates for the weather, units, the note title, the horoscope feed, and how many headlines
- `extensions` — `news`, `stocks`, `brief`, `describe` on/off
...
- OCR fallback for scanned PDFs and inaccessible apps (Vision framework — the image describer already reads text in pictures)
- Apple's on-device image model for `D` on macOS 27 (the code is in the tree behind a build flag until the SDK is confirmed)
